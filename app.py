...
if menu == "📟 Input Manual Sensor":
    ...
    with col1:
        ...
        mq135 = st.number_input(
            ...
            min_value=0.0, max_value=4095.0,
            value=270.0, step=1.0,
                ...
                if not MODEL_LOADED:
                    ...
                    if st.button("🚀 Jalankan Prediksi CSV", type="primary"):
                        with st.spinner(f"Memproses {len(df_raw):,} baris data..."):
                            ...
                            if baris_null > 0:
                                st.warning(f"⚠️ {baris_null} baris mengandung nilai non-numerik dan akan dilewati.")
                                X = X.dropna()
                            ...
                            df_hasil['Label_Prediksi']     = [KELAS_LABEL.get(p, p) for p in preds]
